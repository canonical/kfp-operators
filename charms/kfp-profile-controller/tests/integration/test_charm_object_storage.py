...
async def test_s3_secret_resources(ops_test: OpsTest):
    """Tests the s3 credentials Secret reflects s3-credentials, not minio, data.

    The credentials are randomly generated by microceph, so we cannot assert
    exact values. Instead, assert they are non-empty and differ from the known
    minio config values.
    """
    lightkube_client = lightkube.Client()
    secret = lightkube_client.get(
    ...
    access_key = b64decode(secret.data["MINIO_ACCESS_KEY"]).decode("utf-8")
    secret_key = b64decode(secret.data["MINIO_SECRET_KEY"]).decode("utf-8")
    assert access_key and access_key != MINIO.config["access-key"]
