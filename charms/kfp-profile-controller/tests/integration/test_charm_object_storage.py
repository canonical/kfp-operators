...
async def test_remove_object_storage_relation(ops_test: OpsTest):
    """Remove the object-storage relation cleanly.

    With no storage relation in place the charm is expected to go into the
    blocked state due to the s3-relations-conflict-detector component.
    """
    await ops_test.model.applications[CHARM_NAME].remove_relation(
        f"{CHARM_NAME}:object-storage",
        f"{MINIO.charm}:object-storage",
    )
    await ops_test.model.wait_for_idle(apps=[CHARM_NAME], status="blocked", timeout=60 * 10)


async def test_deploy_and_integrate_s3_integrator(ops_test: OpsTest):
    """Deploy s3-integrator and integrate it as the object storage backend."""
    ...
    await ops_test.model.wait_for_idle(
        apps=[CHARM_NAME, S3_INTEGRATOR.charm], status="active", timeout=60 * 10
    )


async def test_s3_secret_resources(ops_test: OpsTest):
    """Tests that the s3 credentials Secret is recreated with non-empty data.

    The Secret is deleted beforehand to ensure the assertion is operating on
    a Secret created from the s3-credentials relation, rather than
    on any leftover value from the previous object-storage relation.
    ...
    # Delete the leftover Secret so the charm has to recreate it from s3
    lightkube_client.delete(
        Secret,
        name=f"{CHARM_NAME}-minio-credentials",
        namespace=ops_test.model_name,
    )

    # Wait for the charm to reconcile and recreate the Secret
    await ops_test.model.wait_for_idle(apps=[CHARM_NAME], status="active", timeout=60 * 10)

    secret = lightkube_client.get(
        Secret,
        f"{CHARM_NAME}-minio-credentials",
        namespace=ops_test.model_name,
    )
