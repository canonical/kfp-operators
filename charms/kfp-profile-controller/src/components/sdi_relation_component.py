...
class SdiRelationGetterComponent(Component):
    ...
    def get_data(self) -> Union[List[dict], dict]:
        ...
        try:
            ...
            # not incomplete
            msg = f"Got ValidationError when interpreting data on relation {self._relation_name}: {val_error}"  # noqa: E501
    ...
    def validate_data(self, data: List[dict]):
        ...
        if self._minimum_related_applications == self._maximum_related_applications:
            error_msg = (
                f"Expected data from exactly {self._minimum_related_applications} related applications - "  # noqa: E501
                f"got {len(data)}."
            )
        else:
            error_msg = (
                f"Expected data from {self._minimum_related_applications}-{self._maximum_related_applications} "  # noqa: E501
