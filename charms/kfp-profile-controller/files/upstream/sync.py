...
def get_settings_from_env(
    ...
    settings["minio_namespace"] = minio_namespace or os.environ.get(
        "MINIO_NAMESPACE", "kubeflow"
    )
    ...
    settings["metadata_grpc_service_port"] = (
        ...
        or os.environ.get("METADATA_GRPC_SERVICE_PORT", "8080")
    )

    settings["ambient_mesh_enabled"] = (
        os.environ.get("AMBIENT_ENABLED", "false") == "true"
    )
...
def server_factory(
    ...
    class Controller(BaseHTTPRequestHandler):
        def sync(self, parent, attachments):
            ...
            # if ambient is enabled, then we create the AuthorizationPolicy for API to
